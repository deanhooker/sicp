...
;; Exercise 5.31
;; In the interpreter we always save and restore
;; the env register around the evaluation of the
;; operater and operands (except the final operand),
;; saves and restores argl around the evaluation of
;; each operand, and saves and restores proc around
;; the evaluation of the operands.

;; (f 'x 'y)
;; Save and restore of the env, argl and proc
;; registers is unnecessary.

;; ((f) 'x 'y)
;; Save and restore of the argl and proc registers
;; is unnecessary.

;; (f (g 'x) y)
;; Save and restore of each register is necessary.

;; (f (g 'x) 'y)

;; Exercise 5.32
;; Adding each compilation case to the interpreter
;; will not gain all of the compilation advantages,
;; since it will effectively compile on each run,
;; instead of once.

;; Exercise 5.33
;; There are the same number of instructions in
;; both factorial and faction-alt. In the first
;; the argl register is saved and restored, and in
;; the second, the env register must be saved and
;; restored.
